...
$(function () {
    ...
    let genderOptionsValues = '<option value="Select Gender" disabled selected>בחר</option>';
    ...
    $("#addUserFormsubmit").click(function (event) {
        ...
        if ($('#addUserForm').valid()) {            
            ...
            //    postData.set("BillingAddress.Country", country[0].name)
                postData.set("BillingAddress.Country", 'try that')
            //}
            ...
            postData.set("ShippingAddress.Country", 'try that')
            //}
...
//            countries = result;
//            if (countries) {
//                let optionsValues = '<option value="Select Country" disabled selected>בחר</option>';
...
function addUser(postData) {    
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ...
            Swal.fire({
                title: 'שגיאה!',
                text: 'לא היתה אפשרות ליצור את המשתמש',
...
function EditUser(postData) {
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ...
            Swal.fire({
                title: 'שגיאה!',
                text: 'לא היתה אפשרות לערוך את פרטי המשתמש',
        ...
        $.ajax({
            ...
            error: function (error) {
                Swal.fire({
                    type: 'error',
                    title: 'אופס...',
                    text: 'הבקשה לא הושלמה, נסה שוב, שים לב שאין אפשרות למחוק משתמש שלא אישר הרשמה בכתובת המייל'
                })
            }
        });
    }
...
function addNewUser() {
    ...
    $("#AddUserModal .modal-title").html("משתמש חדש");
}
...
function editUserById(value) {
    ...
    $("#EditUserModal .modal-title").html("עריכת שם משתמש");
}

function deleteUser(value) {
    var deluser = $(value).data('deluser');
    $("#DeleteUserModal").modal('show');
    $("#DeleteUserModal .modal-title").html("אישור מחיקה");
    $("#DeleteUserModal .modal-body").html("בטוח שברצונך למחוק את המשתמש בשם:  " + "<strong class='text-danger'><span id='toDeleteUr'>" + deluser + "</span></strong>");
}
/* TABULATOR CODE */
var table = new Tabulator("#usersTable", {
    ...
    paginationSize: 10,         //allow 7-10 rows per page of data
    ...
    columns: [                //define the table columns
        {
            title: "שם משתמש", align: "right", field: "username"},
        { title: "שם פרטי", align: "right", field: "firstname" },
        { title: "שם משפחה", align: "right", field: "lastname" },
        { title: "כתובת מייל", align: "right", field: "email" },
        //{
        //    title: "תמונת פרופיל", align: "right", field: "profilePic", formatter: "image", formatterParams: {
        //        height: "50px",
        //        width: "50px"
        //    }
        //},
        { title: "הפרופיל הושלם", align: "right", field: "isProfileComplete", formatter: "tickCross"},
        { title: "פעיל", align: "right", field: "isActive", formatter: "tickCross" },

        {
            title: "פעולות", align: "right", formatter: function (cell) {
                var userName = cell.getData().username;
                var firstName = cell.getData().firstname;
                var lastName = cell.getData().lastname;
                //Create the button element
                var newEditRow = "<div class='btn-group center rtl' role='group' aria-label='Perform Actions'>" +
                    "<button type='button' name='Delete' data-deluser='" + userName + "'  class='btn btn-danger btn-sm center' onclick='deleteUser(this)' " +
                    ">" +
                    "<span>" +
                    ...
                    "</span>" +
                    "</button>" +
                    "<button type='button' name='Edit'  class='btn btn-primary btn-sm center' onclick='editUserById(this)' " +
