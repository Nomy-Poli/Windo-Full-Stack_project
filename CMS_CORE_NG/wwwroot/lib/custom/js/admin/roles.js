...
var table = new Tabulator("#userRoleTable", {
    ...
    columns: [                //define the table columns
        { title: "שם", field: "name", align: "right" },
        {
            title: "טיפול", field: "handle", align: "right", formatter: function (cell) {
                return "@" + cell.getData().handle;
            }
        },
        {
            title: "אייקון", field: "roleIcon", align: "right", formatter: function (cell) {
        ...
        { title: "פעיל", field: "isActive", align: "right", formatter: "tickCross" },
        {
            title: "פעולות", align: "right", formatter: function (cell) {
                ...
                var newEditRow = "<div class='btn-group' role='group' aria-label='Perform Actions'>" +
                    ...
                    ">" +
                    "<span>" +
                    "<i class='fa fa-eye' title='deatails'>" +
                    "</i>" +
                    "</span>" +
                    "</button>" +
                    ...
                    ">" +
                    "<span>" +
                    "<i class='fa fa-trash m-1' title='delete'>" +
                    "</i>" +
                    "</span>" +
                    "</button>" +
                    ...
                    ">" +
                    "<span>" +
                    "<i class='fa fa-edit m-1' title='edit'>" +
                    "</i>" +
                    "</span>" +
                    "</button>" +              
                    "</div>";
    ...
    $.ajax({
        ...
        error: function (error) {
            Swal.fire({
                type: 'error',
                title: 'אופס...',
                text: 'לא ניתן היה להשלים את הבקשה, בבקשה נסה שוב מאוחר יותר'
            })
        }
    });
}
...
function loadRoleView(value) {
    ...
    $("#ViewRoleModal .modal-title").html("פרטי הרמות");
...
function loadPermissionTypeModal() {
    ...
    $("#PermissionTypeModal .modal-title").html("הוספת סוג הרשאה");
...
function deleteRoleById(value) {
    ...
    $("#DeleteRoleModal .modal-title").html("מחיקת הרשאה");
    $("#DeleteRoleModal .modal-body").html("האת את/ה רוצה למחוק הרשאה? : " + "<strong class='text-danger'><span>" + delRoleName + " with Id</span><span id='toDeleteRL'>" + delId + "</span></strong>");
}
...
function addRoleById() {
    ...
    $("#AddRoleModal .modal-title").html("הוספת רמה חדשה");
...
function editRoleById(value) {
    var permissions = $(value).data('permissions');
    $("#permissionTableEdit tbody").html("שגיאה...");
    ...
    $("#EditRoleModal .modal-title").html("עדכון פרטי הרמות");
    $("#roleErrorEdit").text("");
...
function confirmAddRole(event) {
    ...
    if (ApplicationRole.RoleName !== "") {
        ...
        $("#roleError").text("הכנס בבקשה שם של רמה");
    }
}
    ...
    if (ApplicationRole.RoleName !== "") {
        $.ajax({
            ...
            success: function (result) {
                ...
                Swal.fire(
                    'עבר בצלחה!',
                    'Role : ' + ApplicationRole.RoleName + ' עודכן.',
                    'success'
                );
                GetUserRoles();
            },
            error: function (error) {
                Swal.fire({
                    type: 'error',
                    title: 'אופס...',
                    text: 'שגיאה : לא ניתן היה למלא את הבקשה, נסה שוב מאוחר יותר'
                })
            }
        });
        ...
        $("#roleErrorEdit").text("אנא מלא שם לתפקיד.");
    }
...
function addRolePermissionType(event) {
    ...
    if (PermissionTypeName !== "") {
        ...
        $("#PermissionTypeNameError").text("אנא מלא סוג הרשאה.");
    }
}
