...
namespace AuthService
{
    public class AuthSvc : IAuthSvc
    {
        ...
        public AuthSvc(UserManager<ApplicationUser> userManager, IConfiguration configuration,
        ...
        public async Task<TokenResponseModel> Auth(LoginViewModel model)
        {
            ...
            try
            {
                ...
                if (user == null) return CreateErrorResponseToken("הבקשה אינה נתמכת", HttpStatusCode.Unauthorized);
                ...
                if (!await _userManager.CheckPasswordAsync(user, model.Password))
                {
                    ...
                    return CreateErrorResponseToken("הבקשה אינה נתמכת", HttpStatusCode.Unauthorized);
                }
                ...
                if (!await _userManager.IsEmailConfirmedAsync(user))
                {
                    ...
                    await _activitySvc.AddUserActivity(activityModel);
                    Log.Error("שגיאה : כתובת המייל לא אומתה בשביל {user}", user.UserName);
        ...
        private async Task<TokenResponseModel> GenerateNewToken(TokenRequestModel model)
        {
            try
            {
                ...
                if (user != null && (await _userManager.CheckPasswordAsync(user, model.Password) || (model.Password== "InsertWithGoogleProvider")) && user.UserRole != "Administrator")
                {
                    ...
                    if (!await _userManager.IsEmailConfirmedAsync(user))
                    {
                        return CreateErrorResponseToken("כתובת המייל לא אומת", HttpStatusCode.Unauthorized);
                    }
                    ...
                    _cookieSvc.SetCookie("userRole", user.UserRole, Convert.ToInt32(refreshTokenExpireTime), false, false);
                    _cookieSvc.SetCookie("user_id", accessToken.UserId, Convert.ToInt32(refreshTokenExpireTime));
                   //_cookieSvc.SetCookie("user_id_normaly", user.Id, Convert.ToInt32(refreshTokenExpireTime), false, false);
                    _cookieSvc.SetCookie("email", user.Email, Convert.ToInt32(refreshTokenExpireTime), false, false);
                    if (user.Firstname != null)
                    {
                        _cookieSvc.SetCookie("firstName", user.Firstname, Convert.ToInt32(refreshTokenExpireTime), false, false);
                    }
                    if (user.Lastname != null)
                    {
                        _cookieSvc.SetCookie("lastName", user.Lastname, Convert.ToInt32(refreshTokenExpireTime), false, false);
                    }
                    return accessToken;

                }
                return CreateErrorResponseToken("שם משתמש / סיסמה אינם תקינים", HttpStatusCode.Unauthorized);
            }
            catch (Exception ex)
            {
                Log.Error("An error occurred while seeding the database  {Error} {StackTrace} {InnerException} {Source}",
        ...
        /*GoogleJsonWebSignature.Payload ומחזירים תשובה מסוג client שבתוכו פרמטרים, אותם אנחנו שולחים מה externalAuth פונק זו מקבלת 
        האובייקט שמוחזר מכיל בתוכו אינפורמציה על משתמשים שונים
        של האפליקציה בה אנחנו משתמשים client id   שבתוכו יש את ה Audience  שמכיל בתוכו מאפיין ששמו  ValidationSettings  קודם לכן, נגדיר      
        token עם המאפיינים האלו אנחנו יכולים לאבטח את תקינות של     
         */
        public async Task<GoogleJsonWebSignature.Payload> VerifyGoogleToken(ExternalAuthModel externalAuth)
        {
            //אנחנו מגדירים את ההגדרות של ההרשאות
            try
            {
                ...
                var payload = await GoogleJsonWebSignature.ValidateAsync(externalAuth.IdToken, setting);
                return payload;
            }
            catch (Exception)
            {
        ...
        //for insert with google
        public async Task<TokenResponseModel> GenerateNewToken(ApplicationUser user)
        {
            var model = new TokenRequestModel
            {
                UserName = user.UserName,
                //Password = user.PasswordHash,
                Password = "InsertWithGoogleProvider",
                GrantType = "password",
            };
            var jwtToken = await Auth(model);
            if (!jwtToken.TwoFactorLoginOn || jwtToken.ResponseInfo.Message == "כתובת המייל לא אומת")
                return jwtToken;
            return null;
        }
    }
