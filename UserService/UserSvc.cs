...
namespace UserService
{
    public class UserSvc : IUserSvc
    {
        ...
        public async Task<string> OnGetAsync(string userId, string code)
        {
            try
            {
                if (code == null)
                {
                    //return "no code wore provided";
                    return "לא סופק token";
                }
                if (userId == null)
                {
                    return "לא סופק user id";
                }
                ...
                StatusMessage = result.Succeeded ? "ברוכה הבאה אל windo" : "שגיאה במהלך אימות הסיסמה.";
        ...
        public async Task<ResponseObject> RegisterUserAsync(RegisterViewModel model)
        {
            ...
            try
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    Log.Information("App User Created {UserName}", model.Username);
                ...
                //if the registeration not succeeded
                ...
                foreach (var error in result.Errors)
                {
                    switch (error.Code)
                    {
                        case "DuplicateUserName":
                            errorList.Add("DuplicateUserName","שם משתמש זה כבר קיים, אין אפשרות ליצור אותו שוב.");
                            break;
                        case "DuplicateEmail":
                            errorList.Add("DuplicateEmail"," כתובת מייל זאת כבר קיימת, אין אפשרות ליצור אותה שוב.");
                            break;
                        case "PasswordRequiresNonAlphanumeric":
                            errorList.Add("PasswordRequiresNonAlphanumeric"," סיסמה חייבות להכיל לפחות תו אחד שאינו אותיות או מספרים.");
                            break;
                        case "PasswordRequiresLower":
                            errorList.Add("PasswordRequiresLower"," סיסמה חייבת להכיל לפחות אות קטנה אחת באנגלית.");
                            break;
                        case "PasswordRequiresUpper":
                            errorList.Add("PasswordRequiresUpper"," סיסמה חייבת להכיל לפחות אות גדולה אחת באנגלית.");
                            break;
                        case "PasswordRequiresDigit":
                            errorList.Add("PasswordRequiresDigit"," סיסמה חייבת להכיל לפחות ספרה אחת.");
                            break;
                        //case "PasswordRequiresNonAlphanumeric":
                        //    errorList.Add(" סיסמה חייבות להיות לפחות תו אחד שאינו אותיות או מספרים.");
                        //    break;

                        default:
                            errorList.Add("Error","ישנה שגיאה ביצירת המשתמש, אנא בדוק שהפרטים נכונים");
                            break;
                    }
        ...
        public async Task<string> Contact(ContactModel model)
        {
            try
            {
                int affect = 0;
        ...
        /*פונק יצירת משתמש עם google*/
        public async Task<ApplicationUser> LoginWithGoogle(ApplicationUser user, GoogleJsonWebSignature.Payload payload, UserLoginInfo info, UserManager<ApplicationUser> _userManagers)
        {
            //חפש מייל כזה - כדי לדעת האם הוא כבר קיים
            user = await _userManagers.FindByEmailAsync(payload.Email);
            if (user == null)
            {
                //תיצור משתמש ותכניס את המייל כשם משתמש ואת המייל כמייל
                ...
                //תיצור משתמש בdb
                await _userManagers.CreateAsync(user);
                //prepare and send an email for the email confirmation
                //מוסיף את השם משתמש לרמת משתמש
                await _userManagers.AddToRoleAsync(user, "Customer");
                await _userManagers.AddLoginAsync(user, info);
                return user;
            }
            else
            {
        ...
        //create user for the import program
        public async Task<ApplicationUser> CreateSeedUserAsync(RegisterViewModel model)
        {
        ...
        public async Task<int> InsertActivities(ImportResponseModel importResponseModel)
        {
            int id;
            try
            {
                _db.UploadActivitiesHistory.Add(importResponseModel);
                _db.SaveChanges();
                id = importResponseModel.ImportId;
                return id;
            }
            catch (Exception)
            {
